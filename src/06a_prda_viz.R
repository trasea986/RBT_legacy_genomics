...
write.csv(df_ggplot, "../outputs/figures/final_prda_plot.csv", row.names=FALSE)
...
man_plot <- ggplot(df_ggplot, aes(x=BPcum, y=correlation)) +
  ...
  ggtitle('pRDA Correlation Value') +
...
ggsave(filename = "../outputs/figures/pRDA_man.png", plot = man_plot, height = 6, width = 10, units = "in")
