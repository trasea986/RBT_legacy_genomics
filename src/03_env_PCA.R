...
#after talking with Kim, removing duplicate rows which represent different time points

#2, 4, 6, 8 18 are the repeated time point rows
env_final1 <- env_final[c(-2, -4, -6, -8, -18),]

#also drop elevation due to issues in later GEA steps


matrix <- env_final1[, 4:12]
...
PCA1 <- 4.360643e+01
PCA2 <- 3.403831e+01
# PCA3 <- 7.149347e+00 use this if wanting axes 3 and 4
# PCA4 <- 6.723630e+00
...
matrix$Ecotype <- env_final1$Ecotype
matrix$Stream <- env_final1$Stream

matrix <- separate(matrix, Stream, into = c("Stream", "Hydro"), sep = '-')

#rename the Mann creek to "a" and "b"
matrix$Stream <- c("Little Jacks Creek","Big Jacks Creek","Duncan Creek" ,"Williams Creek","Keithly Creek","Johnson Creek","Little Weiser River ","Dry Creek","Fawn Creek","Mann Creek (a)","Mann Creek (b)","Trail Creek","South Callahan Creek")
...
vars12 <- ggplot() +
  ...
  geom_segment(data = pca.vars, aes(x = 0, xend = Dim.1, y = 0, yend = Dim.2),arrow = arrow(length = unit(0.025, "npc"), type = "open"),
               lwd = 1) + 
  geom_text_repel(data = pca.vars, aes(x = Dim.1*1.15, y =  Dim.2*1.15,label = c(colnames(matrix[,1:9]))), size = 4) +
    xlab(paste("PC1 ", PCA1, "%", sep = '')) + 
    ylab(paste("PC2 ", PCA2, "%", sep = '')) +
    coord_equal() +
    theme_bw(base_size = 13) +
...
plot_pca_final <- plot_grid(pc12_points, vars12,
          labels = c("A)", "B)"), nrow = 1, rel_heights = c(1,1))


ggsave('../outputs/figures/pca_env.png', plot = plot_pca_final, width = 35, height = 20, units = "cm")
