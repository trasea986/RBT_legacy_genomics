...
vars12 <- ggplot() +
  ...
  geom_segment(data = pca.vars, aes(x = 0, xend = Dim.1, y = 0, yend = Dim.2),arrow = arrow(length = unit(0.025, "npc"), type = "open"),
               lwd = 1) + 
  geom_text(data = pca.vars, aes(x = Dim.1*1.15, y =  Dim.2*1.15,label = c(colnames(matrix[,1:8]))), check_overlap = F, size = 3) +
    xlab(paste("PC1 ", PCA1, "%", sep = '')) + 
    ylab(paste("PC2 ", PCA2, "%", sep = '')) +
    coord_equal() +
    theme_bw(base_size = 13) +
...
vars34 <- ggplot() +
  ...
  geom_segment(data = pca.vars, aes(x = 0, xend = Dim.3, y = 0, yend = Dim.4),arrow = arrow(length = unit(0.025, "npc"), type = "open"),
               lwd = 1) + 
  geom_text(data = pca.vars, aes(x = Dim.3*1.15, y =  Dim.4*1.15,label = c(colnames(matrix[,1:8]))), check_overlap = F, size = 3) +
  xlab(paste("PC3 ", PCA3, "%", sep = '')) + 
  ylab(paste("PC4 ", PCA4, "%", sep = '')) +
  coord_equal() +
  theme_bw(base_size = 13) +
...
ggsave('./outputs/pca.png', plot = last_plot(), width = 35, height = 35, units = "cm")
