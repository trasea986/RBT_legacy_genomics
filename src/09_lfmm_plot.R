...
###looks like one output the columns were different. below code fixes column order.
...
write.csv(df, paste("../outputs/final_lfmm_K9_",df[1,3],".csv", sep = ''), row.names=FALSE)

#write the candidate snp files using different p value thresholds
cand_SNP_K9 <- subset(df, pval <= 0.05)
write.csv(cand_SNP_K9, paste("../outputs/final_lfmm_K9_05_",df[1,3],".csv", sep = ''), row.names=FALSE)
cand_SNP_K9 <- subset(df, pval <= bonferroni)
write.csv(cand_SNP_K9, paste("../outputs/final_lfmm_K9_Bon_",df[1,3],".csv", sep = ''), row.names=FALSE)
cand_SNP_K9 <- subset(df, pval <= criticalValue)
write.csv(cand_SNP_K9, paste("../outputs/final_lfmm_K9_BH_",df[1,3],".csv", sep = ''), row.names=FALSE)
...
write.csv(df_ggplot, paste("../outputs/figures/final_lfmm_K9_plot_",df[1,3],".csv", sep = ''), row.names=FALSE)
...
ggsave(filename = paste("../outputs/figures/", plot_title,".png", sep = ""), plot = man_plot, height = 6, width = 10, units = "in")
